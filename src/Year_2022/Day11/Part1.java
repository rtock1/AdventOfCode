...
public class Part1 {
    public String start(String input) {
        ...
        ArrayList<Integer>[] itemsByMonkey = new ArrayList[monkeys.length];
        ...
        int[] examineNumbers = new int[monkeys.length];
        for (int i=0;i<monkeys.length;i++){
            itemsByMonkey[i] = new ArrayList<>();
            for (int a: Array.strToInt(monkeys[i].split("\\n")[1].split(": ")[1].split(", "))) {
        ...
        for (int cycle = 0; cycle<20; cycle++){
            for (int a=0;a<monkeys.length;a++) {
                ...
                for (int item = 0;item<repeatLength;item++) {
                    examineNumbers[a]++;
                    int num1 = Integer.parseInt(operationsByMonkey[a].split(" ")[0].replaceAll("(old)", itemsByMonkey[a].get(0) + ""));
                    int num2 = Integer.parseInt(operationsByMonkey[a].split(" ")[2].replaceAll("(old)", itemsByMonkey[a].get(0) + ""));;
                    char op = operationsByMonkey[a].split(" ")[1].charAt(0);
                    switch (op) {
                        case '*':
                            itemsByMonkey[a].set(0, (num1 * num2) / 3);
                            break;
                        case '+':
                            itemsByMonkey[a].set(0, (num1 + num2) / 3);
                            break;
